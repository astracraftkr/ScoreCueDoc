...
- **사업자 정보 기재 완료** (2026-08-14). 계약 당사자는 **Astra Craft**(대표 박효진)이며, 상호·대표자·
  사업자등록번호·주소·통신판매업 신고번호를 약관 제1조와 전 페이지 푸터에 표시한다. 대외 연락처는
  `astracraft.kr@gmail.com`으로 통일했다(구 `stellarvibelabs@gmail.com`은 폐기).
  - ⚠️ **주소를 도로명까지만(`경기도 오산시 큰말길`) 적었다.** 홈 오피스라 상세주소를 공개하지 않기
    위해서이고 **D-U-N-S 등록 주소와 일치시킨 값**이다. 다만 「전자상거래법」상 사업장 소재지는 통상
    상세주소까지 요구하므로 검토가 필요하다. 주소를 바꾸려면 사업자등록·통신판매업 신고·D&B 프로필을
    함께 정정해야 하고, Play·Apple의 조직 인증이 그 셋을 대조한다.
  - ⚠️ **문의 전화번호가 개인 휴대폰(010-2501-7357)이다** (2026-08-14, 사용자 지시 "일단"). 약관·푸터에
    공개 표시되고 **Play Console 한국 필수 정보에도 같은 번호가 들어가 스토어 등록정보 하단에 노출된다.**
    영구히 쓸 번호인지 재검토할 것 — 대표번호나 안심번호 서비스로 바꾸려면 문서 3곳(`ko/terms.md`,
    `en/terms.md`, `_layouts/default.html`)과 Play Console을 함께 고친다.
